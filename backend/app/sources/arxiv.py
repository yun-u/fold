...
class Arxiv:
    ...
    def __init__(
        ...
        redis_key_timeout: datetime.timedelta = datetime.timedelta(days=1),
    ) -> None:
        self.redis_client = (
            redis_client if redis_client else Redis(connection_pool=redis_pool.pool)
        )
        self.pdf_dir = Path(pdf_dir)
        self.redis_key_timeout = redis_key_timeout
    ...
    def fetch_metadata(self, arxiv_id: str) -> ArxivMetadata:
        ...
        if self.validate_arxiv_id_versioned(arxiv_id) is None:
            ...
            pipe.expire(self.get_arxiv_metadata_key(arxiv_id), self.redis_key_timeout)
            pipe.execute()

            logging.info(
                f"[{arxiv_id}] Metadata for unversioned ArXiv ID has been cached for {timedelta_to_human_readable(self.redis_key_timeout)}."
            )
